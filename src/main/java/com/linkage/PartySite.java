...
public class PartySite {
    ...
    public static void main( String[] args ) {
        ...
        try (MongoClient mongoClient = MongoClients.create(settings)) {
            ...
            List<String> siteIDs = Arrays.asList("party1", "party2"); // List of siteIDs

            List<Bson> pipeline = new ArrayList<>();

            // Match stage
            pipeline.add(Aggregates.match(Filters.in("siteID", siteIDs)));

            // Lookup stage
            pipeline.add(Aggregates.lookup("PSALinkage", "siteID", "partySite.siteID", "linkage"));
